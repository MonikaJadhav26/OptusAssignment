...
class ApiClient {
  ...
  /// "Result" type is used for fecting response success or failure //Result<EmployeeDataModel, Error>
     func getAllCityTemperatureList(completion: @escaping (Result<CityWether, Error>) -> Void) {
      let urlString = "\(WeatherApi.apiBaseUrl)/weather?id=\("2147714")&units=metric&appid=\(WeatherApi.apiKey)"
